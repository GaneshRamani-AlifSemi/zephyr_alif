...
static int start_next_packet(const struct device *dev)
{
	...
	if (dev_data->xfer.xfer_mode == MSPI_DMA) {
		...
		if (dev_data->standard_spi) {
			...
			if (dev_data->xfer.cmd_length) {
				/* Command-only TX packets are framed as 16-bit words to
				 * mirror the legacy HAL OSPI path. The opcode itself is
				 * written directly, matching the HAL driver's opcode TX.
				 */
