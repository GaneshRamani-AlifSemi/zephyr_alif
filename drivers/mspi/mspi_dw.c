...
#if SUPPORTS_XIP_SER || DT_HAS_COMPAT_STATUS_OKAY(alif_designware_ssi)
...
static void apply_timing_config(const struct device *dev)
{
	...
	uint8_t vendor_drive_edge = vendor_specific_ddr_drive_edge(dev);

	if (vendor_drive_edge != 0U) {
		write_txd_drive_edge(dev, vendor_drive_edge);
...
static int start_next_packet(const struct device *dev)
{
	...
	if (dev_data->standard_spi && !data_only_packet) {
		...
		if (packet->num_bytes == 0 && packet->dir == MSPI_TX) {
			/* Command-only TX packets are used for opcode-style transfers
			 * like WREN and RESET. Use a single 8-bit frame here so the
			 * opcode goes out as one byte on the wire, matching the
	 ...
	 * is enabled or busy. Select the target before SSIENR is asserted.
	 */
	if (DT_HAS_COMPAT_STATUS_OKAY(alif_designware_ssi)) {
