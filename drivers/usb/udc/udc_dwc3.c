...
//struct udc_dwc3_data  __alif_sram0_section udc0_dwc3_priv;
//struct udc_dwc3_data  udc0_dwc3_priv;
...
void dwc3_connect_cb(USB_DRIVER *Drv)
{
	...
	if (ep && ep->stat.enabled)
	{
		...
		status =  usbd_endpoint_create(&priv->drv, ep_type, ep_num, ep_dir, ep->mps, ep_interval);
		if(status != 0)
		{
			return ;
		}
	...
	/*ret = udc_ep_enable_internal(priv->dev, USB_CONTROL_EP_OUT, USB_EP_TYPE_CONTROL, cfg->ep0_mps, 0);
	if (ret)
	{
	...
	ret = udc_ep_enable_internal(priv->dev, USB_CONTROL_EP_IN, USB_EP_TYPE_CONTROL, cfg->ep0_mps, 0);
	if (ret)
	{
...
int udc_dwc3_init(const struct device *dev)
{
	...
	status = usbd_initialize(&priv->drv);
	if(status)
	{
...
static int udc_dwc3_enable(const struct device *dev)
{
	...
	const struct udc_dwc3_config *cfg = dev->config;
	uint32_t status;
	status = usbd_connect(&priv->drv);
	if(status != 0)
	{
		printk("dwc3 controller enable failed\n");
	}
	status = udc_ep_enable_internal(dev, USB_CONTROL_EP_OUT, USB_EP_TYPE_CONTROL, cfg->ep0_mps, 0);
	if (status)
	{
		LOG_ERR("Failed enabling ep 0x%02x", USB_CONTROL_EP_OUT);
		return status;
	...
	status = udc_ep_enable_internal(dev, USB_CONTROL_EP_IN, USB_EP_TYPE_CONTROL, cfg->ep0_mps, 0);
	if (status)
	{
		LOG_ERR("Failed enabling ep 0x%02x", USB_CONTROL_EP_IN);
		return status;
...
static int udc_dwc3_ep_enable(const struct device *dev, struct udc_ep_config *ep_cfg)
{
	...
	status =  usbd_endpoint_create(&priv->drv, ep_type, ep_num, ep_dir, ep_cfg->mps, ep_interval);
	if(status != 0)
	{
		return status;
	}
...
static int udc_dwc3_ep_set_halt(const struct device *dev, struct udc_ep_config *cfg)
{
	...
	status = usbd_ep_stall(&priv->drv, ep_num, ep_dir);
	if(status != 0)
	{
		return status;
	}
...
static int udc_dwc3_ep_clear_halt(const struct device *dev, struct udc_ep_config *cfg)
{
	...
	ep_num = EP_NUM(cfg->addr);
	ep_dir = (cfg->addr & USB_EP_DIR_MASK)? USB_DIR_IN: USB_DIR_OUT;
	status =  usbd_ep_clearstall(&priv->drv, ep_num, ep_dir);
	if(status != 0)
	{
		return status;
	}
...
static int udc_dwc3_tx(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if(ep_num == 0)
	{
		ret = usbd_ep0_send(&priv->drv, ep_num, ep_dir, data, len);
		if(ret != 0)
		{
			return ret;
		}
	...
	{
		ret = usbd_bulk_send(&priv->drv, ep_num, ep_dir, data, len);
		if(ret != 0)
		{
			return ret;
		}
...
static int udc_dwc3_rx(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	if(ep_num == 0)
	{
		ret = usbd_ep0_recv(&priv->drv, ep_num, ep_dir, buf->data , buf->size);
		if(ret != 0)
		{
			return ret;
		}
	...
	{
		//bulk_revc ++;
		memset(buf->data, 0x00, 512);
		/*if(bulk_revc == 3)
		{
			printk("break\n");
		}*/
		printk("[USB Device] :buff addr and size : %x %d\n", buf->data, buf->size);
		ret = usbd_bulk_recv(&priv->drv, ep_num, ep_dir, buf->data, buf->size);
		if(ret != 0)
		{
			return ret;
		}
...
static int udc_dwc3_ep_enqueue(const struct device *dev, struct udc_ep_config *epcfg, struct net_buf *buf)
{
...
static void udc_dwc3_irq(const struct device *dev)
{
	...
	usbd_interrupt_handler((USB_DRIVER *)&priv->drv);
}
...
{
	struct usb_setup_packet *setup = &priv->drv.SetupData;
	const struct device *dev = priv->dev;
	struct net_buf *buf;
	int err;
	...
	if (udc_ctrl_stage_is_data_out(dev))
	{
		...
		if (err == -ENOMEM)
		{
			udc_submit_ep_event(dev, buf, err);  //mahesh commented
		}
	}
...
static void handle_data_out(struct udc_dwc3_data *priv, uint8_t ep_num, uint16_t recv_bytes)
{
	...
	if (ep == USB_CONTROL_EP_OUT)//for control endpoint
	{
		if (udc_ctrl_stage_is_status_out(dev))
		{
			udc_ctrl_update_stage(dev, buf);
...
static int udc_dwc3_driver_init0(const struct device *dev)
{
    ...
    //data->caps.addr_before_status = 1;
    if (cfg->speed_idx == 2) {
          data->caps.hs = true;
    }
    ...
    priv->dev = dev;
    priv->irq = USB0_IRQ;
    k_msgq_init(&priv->dwc3_msgq_data, udc_dwc3_msgq_buf_0, sizeof(struct udc_dwc3_msg), CONFIG_UDC_DWC3_MAX_QMESSAGES);
