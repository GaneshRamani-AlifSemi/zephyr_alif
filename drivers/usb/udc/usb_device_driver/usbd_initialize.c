...
uint8_t __alif_ns_section event_buff[4096];
uint8_t __alif_ns_section scratch_pad[4096];
//uint8_t  __alif_sram0_section event_buff[4096];
//uint8_t  __alif_sram0_section scratch_pad[4096];
//uint8_t   event_buff[4096];
...
static void usbd_InitializeEps(USB_DRIVER *drv)
{
    ...
    for (i = 0U; i < 4; i++)
    {
        ...
        ep_num = (i << 1U) | USB_DIR_OUT;//need to uncomment
        drv->eps[ep_num].phy_epnum = ep_num;
        drv->eps[ep_num].ep_dir = USB_DIR_OUT;//need to uncomment
        drv->eps[ep_num].ep_resource_index = 0U;
    }
    for (i = 0U; i < 4; i++)
    {
    	drv->eps[ep_num].ep_status = 0U;
        ep_num = (i << 1U) | USB_DIR_IN;//need to uncomment
        drv->eps[ep_num].phy_epnum = ep_num;
        drv->eps[ep_num].ep_dir = USB_DIR_IN;//need to uncomment
        drv->eps[ep_num].ep_resource_index = 0U;
    }
    for( i = 0; i < 8; i++)
    {
    	memset(&drv->eps[i].EpTrb[0], 0x00, 144);//required in zephyr

    }
   // memset(&drv->endp0_trb, 0x00, 16);
 ...
 **/
static uint32_t usbd_SendGenericCmd(USB_DRIVER *drv, uint32_t cmd, uint32_t param)
{
...
/**
  \fn           usbd_SetupScratchpad
  \brief        This function setup the scratchpad buffer.
  \param[in]    pointer to the controller context structure
  \return       On success 0, else error
 **/
static uint32_t usbd_SetupScratchpad(USB_DRIVER *drv)
{
    uint32_t ret;
    ...
    ret = usbd_SendGenericCmd(drv, USB_DGCMD_SET_SCRATCHPAD_ADDR_LO, param);
    if (ret !=  0)
    {
#ifdef  DEBUG
    ...
    ret = usbd_SendGenericCmd(drv, USB_DGCMD_SET_SCRATCHPAD_ADDR_HI, param);
    if (ret != 0)
    {
#ifdef DEBUG
...
static void usbd_phy_setup(USB_DRIVER *drv)
{
    ...
    switch(drv->hsphy_mode)
    {
        ...
        case USB_PHY_INTERFACE_MODE_UTMIW:
            reg &= ~(USB_GUSB2PHYCFG_PHYIF_MASK | USB_GUSB2PHYCFG_USBTRDTIM_MASK | USB_GUSB2PHYCFG_ULPI_UTMI );
...
static void usbd_setup_global_control(USB_DRIVER *drv)
{
    uint32_t reg;
    reg = drv->regs->GCTL;
    /* Disable Clock Gating (DsblClkGtng) This bit is set to 1 and the controller is in Low Power mode,
     * internal clock gating is disabled, You can set this bit to 1 after Power On Reset   */
...
static void usbd_set_incr_burst_type(USB_DRIVER *drv)
{
   ...
   // reg &= ~USB_GSBUSCFG0_INCRBRST_MASK;
...
static uint32_t usbd_device_init(USB_DRIVER *drv)
{
   ...
   // uint32_t retries;
    ...
    usbd_InitializeEps(drv);
    /* allocate and setup scratchpad buffers  */
    ret = usbd_SetupScratchpad(drv);
    if(ret != 0)
    {
        drv -> event_buf = NULL;
        return ret;
...
int32_t  usbd_initialize(USB_DRIVER *drv)
{
   ...
   // usb_ctrl2_phy_power_on_reset_clear();
    ...
    drv->usbd_device_reset_cb = dwc3_reset_cb;
    drv->usbd_connect_cb = dwc3_connect_cb;
    drv->usbd_setupstage_cb = dwc3_setupstage_cb;
    drv->usbd_disconnect_cb = dwc3_disconnect_cb;
    drv->usbd_data_in_cb = dwc3_data_in_cb;
    drv->usbd_data_out_cb = dwc3_data_out_cb;
   // drv->usbd_status_cb = dwc3_status_cb;
    /* Validate the dwc3 core */
    if (!usb_core_is_valid(drv))
    {
#ifdef  DEBUG
        printf("this is not a DesignWare dwc3 DRD Core\n");
#endif
        ret = USB_NO_DEVICE;
        return ret;
    }
