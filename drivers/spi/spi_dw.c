...
{
	const struct spi_dw_config *info = dev->config;
	uint32_t ctrlr0 = 0U, ret, clk_freq;
...
static int spi_dw_suspend(const struct device *dev)
{
	...
	LOG_DBG("Designware SPI driver Suspending..");
	if (pm_device_is_busy(dev)) {
		LOG_DBG("Currently SPI on Transferring ...");
		return -EBUSY;
	}

	LOG_DBG("No SPI Transactions !!!");
...
static int spi_dw_resume(const struct device *dev)
{
	...
	LOG_DBG("Designware SPI driver Resuming");
