...
#define STORAGE_PARTITION_IN_MRAM	DT_SAME_NODE( \
	DT_MEM_FROM_FIXED_PARTITION(DT_NODELABEL(storage_partition)), DT_NODELABEL(mram_storage))
#define MRAM_FLASH_END_ADDR		(DT_REG_ADDR(DT_NODELABEL(mram_storage)) + \
...
/* If storage is moved outside MRAM, keep FLASH_MRAM within the MRAM node. */
#if (!STORAGE_PARTITION_IN_MRAM)
#undef FLASH_MRAM_SIZE
#define FLASH_MRAM_SIZE		(MRAM_FLASH_END_ADDR - FLASH_MRAM_BASE_ADDR)
#endif
